...
public struct ClaudeLogAdapter: Sendable {
  public static let parserVersion = "claude-jsonl-v2"

  public init() {}

  public func metadata(at url: URL) throws -> SessionMetadata {
    guard
      let object = try JSONLReader.firstObject(
        at: url,
        matching: { object in
          object["sessionId"] as? String != nil && object["entrypoint"] as? String != nil
        }),
      let found = Self.metadata(from: object, sourceFile: url.path)
    else {
  ...
  public func parse(at url: URL, rootSessionID: String? = nil) throws -> ParsedSession {
    ...
    let summary = try JSONLReader.forEachObject(
      ...
      if let candidate = Self.metadata(
        from: object, sourceFile: url.path, forcedParentID: rootSessionID),
      ...
      let sessionID =
        (object["sessionId"] as? String) ?? sessionMetadata?.sessionID
        ?? url.deletingPathExtension().lastPathComponent
      let messageID =
        (message["id"] as? String) ?? (object["uuid"] as? String) ?? "line-\(lineNumber)"
      ...
      let recordedTier = usage["service_tier"] as? String

      let record = UsageRecord(
        id: "claude:\(sessionID):\(messageID):\(url.path)",
        provider: .claude,
        sessionID: sessionID,
        ...
        serviceTier: recordedTier.map(Self.serviceTier),
        serviceTierClassificationSource: recordedTier == nil ? nil : .log,
      ...
      if let content = message["content"] as? [[String: Any]] {
        toolCallsByMessageID[messageID] =
          content.filter { $0["type"] as? String == "tool_use" }.count
      }
    }
    ...
    guard summary.validObjects > 0 else {
      throw TokenozaurError.unsupportedFormat("Pusty plik Claude: \(url.lastPathComponent)")
    }
