...
public struct BenchmarkExporter: Sendable {
  ...
  private func csvRow(_ run: BenchmarkRun) -> String {
    ...
    let records = run.lastAnalysis?.sessions.flatMap(\.records) ?? []
    let tiers = Set(records.compactMap(\.serviceTier).map(\.displayName)).sorted()
    columns.append(escape(tiers.joined(separator: "+")))
    let parserVersions = Set(run.lastAnalysis?.sessions.map(\.parserVersion) ?? []).sorted()
    columns.append(escape(parserVersions.joined(separator: "+")))
    let normalizerVersions = Set(run.lastAnalysis?.sessions.compactMap(\.normalizerVersion) ?? [])
      .sorted()
    columns.append(escape(normalizerVersions.joined(separator: "+")))
    let droppedReplay =
      run.lastAnalysis?.sessions.reduce(0) { $0 + ($1.droppedReplayRecordCount ?? 0) } ?? 0
    columns.append("\(droppedReplay)")
    return columns.joined(separator: ",")
  }

  public func markdown(runs: [BenchmarkRun]) -> String {
    var lines = [
      "# Tokenozaur — benchmark export",
      "",
    ...
    for run in runs {
      ...
      let repairCost: String = {
        guard let acceptedValue = accepted?.apiEquivalentUSD,
          let firstValue = first?.apiEquivalentUSD
        else { return "—" }
        return "$" + decimal(acceptedValue - firstValue)
      }()
      lines.append(
        "| \(run.label) | \(run.provider.displayName) | \(firstCost) | \(repairCost) | \(acceptedCost) | \(tokens) | \(run.lastCostReport?.accuracy.rawValue ?? "unavailable") | \(run.lastAnalysis?.sessions.count ?? 0) | \(run.lastAnalysis?.toolCallCount ?? 0) |"
      )
    }
    lines.append("")
    lines.append(
      "> API-equivalent cost nie jest rzeczywistym obciążeniem abonamentu. Tool costs są uwzględnione tylko wtedy, gdy raport mówi o tym wprost."
    )
