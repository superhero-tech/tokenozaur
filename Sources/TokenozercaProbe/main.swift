...
func printReport(provider: Provider, url: URL) {
    ...
    do {
        ...
        if let activityStart = analysis.latestActivityStart(afterInactivity: 30 * 60) {
            ...
            print("activity_cost_usd=\(activityReport.tokenCostUSD.map { NSDecimalNumber(decimal: $0).stringValue } ?? "unavailable")")
...
func printPeriods(only requestedPeriod: String? = nil) {
    ...
    let allCandidates = Provider.allCases.flatMap { discovery.allDesktopSessionFiles(provider: $0) }
    var cache: [String: ParsedSession] = [:]
    print("\n=== Periods ===")
    for (label, start) in starts {
        let candidates = allCandidates.filter { $0.modifiedAt >= start }
        var sessions: [ParsedSession] = []
        for candidate in candidates {
            if let existing = cache[candidate.id] {
            ...
            do {
                ...
                cache[candidate.id] = parsed
                sessions.append(parsed)
            } catch {
                continue
            }
        }
        let root = sessions.first?.metadata ?? SessionMetadata(
            provider: .codex,
            sessionID: "period-summary",
            originator: "tokenozerca-probe",
            sourceFile: "local-history",
            isDesktop: true
        )
        let combined = AnalysisResult(root: root, sessions: sessions)
        let analysis = combined.filteringRecords(from: start, through: now)
        let report = pricing.calculate(analysis)
        print("\(label)_parsed_files=\(sessions.count)")
        print("\(label)_tokens=\(analysis.totalUsage.total)")
        print("\(label)_cost_usd=\(report.tokenCostUSD.map { NSDecimalNumber(decimal: $0).stringValue } ?? "unavailable")")
